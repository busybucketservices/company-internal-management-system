...
// ============================================================
// Busy Bucket · Manager Dashboard — behaviour
// ============================================================

const statActiveAdmin = document.getElementById('statActiveAdmin');
const statAdminRequest = document.getElementById('statAdminRequest');

// ---------------------------------------------------------------
// "Active Admins" / "Admin Requests" chip filter
// ---------------------------------------------------------------
const adminChips = document.querySelectorAll('#adminChips .chip');
const emptyAdminGroup = document.getElementById('emptyAdminGroup');

function applyCurrentAdminGroupFilter() {
  const activeChip = document.querySelector('#adminChips .chip.active');
  if (!activeChip) return;
  const group = activeChip.getAttribute('data-group');
  let anyVisible = false;

  document.querySelectorAll('#adminsDetailList .partner-detail-card').forEach(card => {
    const match = card.getAttribute('data-group') === group;
    card.style.display = match ? 'block' : 'none';
    if (match) anyVisible = true;
  });

  emptyAdminGroup.style.display = anyVisible ? 'none' : 'block';
}

adminChips.forEach(chip => {
  chip.addEventListener('click', () => {
    adminChips.forEach(c => c.classList.remove('active'));
    chip.classList.add('active');
    applyCurrentAdminGroupFilter();
  });
});
...
// ---------------------------------------------------------------
// Top-stat counts — kept in sync with how many cards sit in each group
// ---------------------------------------------------------------
function updateManagerCounts() {
  const activeCount = document.querySelectorAll('#adminsDetailList .partner-detail-card[data-group="active"]').length;
  const requestCount = document.querySelectorAll('#adminsDetailList .partner-detail-card[data-group="request"]').length;
  if (statActiveAdmin) statActiveAdmin.textContent = activeCount;
  if (statAdminRequest) statAdminRequest.textContent = requestCount;
  applyCurrentAdminGroupFilter();
}

// ---------------------------------------------------------------
// Accept an admin request -> moves the card into "Active Admins"
// ---------------------------------------------------------------
function acceptAdminRequest(button) {
  const card = button.closest('.partner-detail-card');
  if (!card) return;

  card.setAttribute('data-group', 'active');
  const tag = card.querySelector('.pd-tag');
  if (tag) {
    tag.className = 'pd-tag fixed';
    tag.textContent = 'Active Admin';
  }

  const statusLine = card.querySelectorAll('.assign-line')[1];
  if (statusLine) statusLine.textContent = 'Online — abhi active hai';

  const actions = card.querySelector('.assign-actions');
  if (actions) {
    const reject = actions.querySelector('.btn-reject');
    const approve = actions.querySelector('.btn-approve');
...
// ============================================================
// Busy Bucket · Managers panel (owner-level "Managers" section)
// — same pattern as Admins above, kept separate so counts/filters
//   for Managers never clash with the Admins ones.
// ============================================================

const statActiveManager = document.getElementById('statActiveManager');
const statManagerRequest = document.getElementById('statManagerRequest');

// ---------------------------------------------------------------
// "Active Managers" / "Manager Requests" chip filter
// ---------------------------------------------------------------
const managerChips = document.querySelectorAll('#managerChips .chip');
const emptyManagerGroup = document.getElementById('emptyManagerGroup');

function applyCurrentManagerGroupFilter() {
  const activeChip = document.querySelector('#managerChips .chip.active');
  if (!activeChip) return;
  const group = activeChip.getAttribute('data-group');
  let anyVisible = false;

  document.querySelectorAll('#managersDetailList .partner-detail-card').forEach(card => {
    const match = card.getAttribute('data-group') === group;
    card.style.display = match ? 'block' : 'none';
    if (match) anyVisible = true;
  });

  if (emptyManagerGroup) emptyManagerGroup.style.display = anyVisible ? 'none' : 'block';
}

managerChips.forEach(chip => {
  chip.addEventListener('click', () => {
    managerChips.forEach(c => c.classList.remove('active'));
    chip.classList.add('active');
    applyCurrentManagerGroupFilter();
  });
});
...
// ---------------------------------------------------------------
// Top-stat counts for the Managers section
// ---------------------------------------------------------------
function refreshManagerStats() {
  const activeCount = document.querySelectorAll('#managersDetailList .partner-detail-card[data-group="active"]').length;
  const requestCount = document.querySelectorAll('#managersDetailList .partner-detail-card[data-group="request"]').length;
  if (statActiveManager) statActiveManager.textContent = activeCount;
  if (statManagerRequest) statManagerRequest.textContent = requestCount;
  applyCurrentManagerGroupFilter();
}

// ---------------------------------------------------------------
// Accept a manager request -> moves the card into "Active Managers"
// ---------------------------------------------------------------
function acceptManagerRequest(button) {
  const card = button.closest('.partner-detail-card');
  if (!card) return;
  ...
  if (tag) {
    tag.className = 'pd-tag fixed';
    tag.textContent = 'Active Manager';
  }
