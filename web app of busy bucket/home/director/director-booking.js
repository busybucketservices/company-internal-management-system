...
// ===================== Team & Requests: Director view — 6-tab system =====================
// Tabs: Managers · Admins · Partners · Manager Requests · Admin Requests · Partner Requests

const teamData = {
  managers: [
    { name:"Rakesh", sub:"Gurugram team · Active" },
    { name:"Sanjay", sub:"Ludhiana team · Active" }
  ],
  admins: [
  ...
  managerRequests: [
    { name:"Anita", sub:"Manager banne ki request · Dehradun", status:"pending", isRequest:true },
    { name:"Suman", sub:"Leave request · 3 din", status:"pending", isRequest:true }
  ],
  adminRequests: [
...
const teamTabLabels = {
  managers:"Managers", admins:"Admins", partners:"Partners",
  managerRequests:"Manager Requests", adminRequests:"Admin Requests", partnerRequests:"Partner Requests"
};
...
function countPendingRequests(){
  const managerPending = teamData.managerRequests.filter(r => r.status === 'pending').length;
  const adminPending = teamData.adminRequests.filter(r => r.status === 'pending').length;
  const partnerPending = teamData.partnerRequests.filter(r => r.status === 'pending').length;
  return managerPending + adminPending + partnerPending;
}
